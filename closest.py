...
import numpy as np
from .covariancetree import CovTreeNode
...
def barycenter(atlas: np.ndarray, tree: CovTreeNode, tri: TriangleThing, c_k: np.ndarray):
    """Computes closest vertex to point and returns distance.
    ...
    t = vert[1]
    u = vert[2]
    sIndex = np.where(treePoints == s)[0][0]
    tIndex = np.where(treePoints == t)[0][0]
    uIndex = np.where(treePoints == u)[0][0]
    ...
    for m in range(atlas.shape[0]):
        # Find qm values
        q = zeta * atlas[m, sIndex] + xi * atlas[m, tIndex] + psi * atlas[m, uIndex]
        np.append(q_mk[m], q)
    return q_mk
    #sean
